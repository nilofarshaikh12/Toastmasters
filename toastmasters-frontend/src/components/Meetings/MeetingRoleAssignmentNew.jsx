...
const MeetingRoleAssignmentNew = ({ 
  open, 
  onClose, 
  meetingCategory, 
  onRolesUpdate, 
  ...
  useEffect(() => {
    ...
    initialRoles.forEach(role => {
      if (role.isCustom) {
        const baseName = role.roleName.replace(/\s+\d+$/, '');
        const existingCustom = customs.find(c => c.name === baseName);
        if (existingCustom) {
          existingCustom.count++;
        } else {
          customs.push({
            id: role.roleId.includes('_') ? role.roleId.split('_')[0] : `custom_${Date.now()}`,
            name: baseName,
        ...
        const baseId = role.roleId.includes('_') ? role.roleId.split('_')[0] : role.roleId;
  ...
  // Generate roles with proper sequential numbering
  const generateRolesArray = () => {
    const allRoles = [];
    
    // Add standard roles with sequential numbering
    ...
    // Add custom roles with sequential numbering
    customRoles.forEach(customRole => {
      for (let i = 1; i <= customRole.count; i++) {
        allRoles.push({
          ...
          isCustom: true,
          instanceNumber: customRole.count > 1 ? i : Date.now() // Use timestamp for unique ID for single custom roles
        });
      }
    });
  ...
  const handleSaveRoles = () => {
    const allRoles = generateRolesArray();
...
export default MeetingRoleAssignmentNew;
